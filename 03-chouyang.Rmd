...
u.test(samp, 5, 1)
z.test(samp,mu = 5,sigma.x = 1)$statistic
```
...
ggplot(NULL, aes(x = samp)) +
  stat_function(fun=dnorm2(-Inf, -1.96), geom="area", fill="red", alpha=0.2) +  
  stat_function(fun=dnorm2(1.96, Inf), geom="area", fill="red", alpha=0.2) +
  stat_function(fun = dnorm,args = list(mean = 5, sd = 1),aes(x = -3:3, color = "标准")) + 
  geom_vline(xintercept = z.test(samp,mu = 5,sigma.x = 1)$statistic) +
